...
public class Sub1 extends TeamRobot {
	...
	static double BULLET_POWER=3;//Our bulletpower.
	static double BULLET_DAMAGE=BULLET_POWER*4;//Formula for bullet damage.
	static double BULLET_SPEED=20-3*BULLET_POWER;//Formula for bullet speed.
	...
	if(!isTeammate(e.getName())){
			...
			if(enemyEnergy>(enemyEnergy=e.getEnergy())){
				...
				if(getEnergy() > 30 ? Math.random() > 300/e.getDistance() : getEnergy() > e.getEnergy()){
					dir=-dir;
				}
			}
			
			if(e.getDistance() > 100){
				if(getX() < 50 && getX() < oldX) 
				dir=-dir;
				else if(getY() < 50 && getY() < oldY)
				dir=-dir;
				else if(getBattleFieldWidth()-getX() < 50 && getX() > oldX)
				dir=-dir;
				else if(getBattleFieldHeight()-getY() < 50 && getY() > oldY)
				dir=-dir;
			}
